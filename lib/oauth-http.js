...
export async function handleOAuthRequest(server, req, res) {
    if (!trusted(req))
        return json(res, 403, { error: 'OAuth controls are available only from the local Harness UI.' });
    const path = new URL(req.url ?? '/', 'http://localhost').pathname;
    try {
        if (req.method === 'GET' && path === PATH) {
            const account = await server.account(false);
            if (account?.type !== 'chatgpt')
                return json(res, 200, { authenticated: false, models: [] });
...
export function oauthRoute(server) {
    return {
        kind: 'prefix',
        path: PATH,
